面向通用人工智能工作流的节点编辑系统设计I. 引言：构想人工智能工作流的统一节点化平台人工智能领域正在经历快速发展，各种模型如大型语言模型（LLM）、视觉语言模型（VLM）以及扩散模型（如Stable Diffusion）在不同应用场景中展现出强大的能力。然而，构建复杂的人工智能工作流通常涉及对这些不同模型的组合使用，以及数据在它们之间的流转和处理。这种复杂性日益增长，对一个统一的可视化编程平台的需求也变得越来越迫切。节点编辑系统通过将复杂的工作流程分解为一系列相互连接的功能模块（节点），提供了一种直观且模块化的构建和管理流程的方式。这种方法不仅提高了开发效率，也使得非专业人士更容易理解和参与到人工智能应用的构建过程中。现有的节点编辑系统在特定领域已经取得了显著的成功，例如ComfyUI在Stable Diffusion工作流的可视化编辑方面表现出色 1。然而，目前还没有一个系统能够真正支持所有类型的AI模型，并提供统一的工作流构建体验。创建一个能够跨越不同模型领域，无缝处理各种数据类型，并提供灵活的自动化流程的平台，将极大地推动人工智能技术的普及和创新。这个报告旨在探讨构建这样一个通用节点编辑系统所需的核心节点类型和功能，并借鉴现有成功系统的经验，为未来的开发提供一个全面的蓝图。II. 现有节点编辑系统的比较分析：识别基础概念和节点类别为了更好地理解通用人工智能工作流平台所需的核心要素，对当前流行的节点编辑系统进行比较分析至关重要。通过研究ComfyUI、Noodl、Node-RED、Blender Geometry Nodes和Unreal Engine Blueprints，我们可以提取出它们的核心节点类型、功能以及成功的设计原则。
